...
export default function EditFieldNote({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchNote = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const note = data.notes.find((n: FieldNote) => n.id === id)
        if (note) {
          setFormData({
            title: note.title,
            slug: note.slug,
            content: note.content,
            author: note.author || 'David Levin',
            feature_image_url: note.feature_image_url,
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formData.append('folder', 'field-notes')
      ...
      {titleContainer && createPortal(
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-lg font-semibold text-foreground">
            ...
            <span>{formData.title || (isNew ? 'New Note' : 'Edit Note')}</span>
          </h2>
        </div>,
        titleContainer
      )}
